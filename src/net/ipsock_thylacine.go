...
func (p *ipStackCapabilities) probe() {
	// netd's /net exposes no /net/iproute; the stack is IPv4 (NET-DESIGN: v1.0
	// is IPv4-A-record only, both the resident lo and the DHCP NIC). Report
	// IPv4 enabled, IPv6 disabled -- the plan9 iproute scan does not apply.
	p.ipv4Enabled = true
	p.ipv6Enabled = false
}
